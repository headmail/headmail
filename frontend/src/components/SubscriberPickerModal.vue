  ...
  <div v-if="visible" class="fixed inset-0 z-50 flex items-center justify-center">
    ...
    <div class="relative bg-white w-full max-w-3xl mx-4 rounded-2xl shadow-xl border border-gray-200 p-6">
      <div class="flex items-center justify-between mb-4">
        <h2 class="text-lg font-semibold text-gray-900">구독자 선택</h2>
      ...
      <div class="space-y-3">
        <div class="flex gap-2">
          <input v-model="search" @input="fetch(1)" type="text" placeholder="구독자 검색..." class="flex-1 px-3 py-2 border rounded" />
        ...
        <div class="max-h-72 overflow-auto border rounded p-2">
          <div v-if="items.length === 0" class="text-sm text-gray-500">구독자가 없습니다.</div>
          <label v-for="it in items" :key="it.id" class="flex items-center justify-between gap-2 p-2 hover:bg-gray-50 rounded">
            <div class="flex items-center gap-2">
              <input type="checkbox" :value="it.id" v-model="selectedLocal" />
              <div>
                <div class="text-sm font-medium text-gray-900">{{ it.name || it.email }}</div>
...
const fetch = async (p = 1) => {
  page.value = p;
  try {
    const resp = await getSubscribers({ page: page.value, limit: limit.value, search: search.value || undefined });
...
const confirm = () => {
  const selectedObjs = selectedLocal.value
    .map((id) => items.value.find((l: Subscriber) => l.id === id))
